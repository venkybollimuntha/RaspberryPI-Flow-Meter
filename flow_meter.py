...
conn1 = sqlite3.connect('/home/pi/Desktop/26_final/krohne_ak.db')
cur = conn.cursor()
        ...
        if pick_val:
            if pick_val == 'today':# not to worry lemee check it
                ...
                if f1 and f2:
                    ...
                    query = "SELECT distinct id,count(id),datetime_val,sum(value) FROM sensor_data where datetime_val BETWEEN '{}' and '{}' GROUP BY strftime('%{}', datetime_val) Order by datetime_val desc".format(start_date,end_date,operator)
...
def pdf_data():
    ...
    try:
        ...
        if pick_val == 'custom':
            ...
            data_list = date_filter_query(pick_val,f1,f2)
        else:
            footer_data['pick_val']=pick_val

            data_list = date_filter_query(pick_val)

        if len(data_list) == 0:
            return '400'

        try:
            print('download PDF method called',data_list)
            # adding database records to a pdf_data_list
            pdf_data_list = [['S.No','Time', 'Flow (m³)']]
            index = 0
            total_flow_sum = 0
            for rec in data_list:
                index += 1
                total_flow_sum += (rec[1]*0.040)
                pdf_data_list.append([index,str(rec[2])[0:19],rec[1]*0.040])

            footer_data['total_flow_sum']= total_flow_sum
            #creating 'waste_water_report.pdf' file
            if str(path.exists('/home/pi/Desktop/26_final/waste_water_report.pdf')) == 'True':
                os.remove("/home/pi/Desktop/26_final/waste_water_report.pdf")
            else:
                open('/home/pi/Desktop/26_final/waste_water_report.pdf', 'a').close()
            ...
            print('Lastpage value',last_page)
            ...
            while pdf_data_len > 0:
                ...
                pdf_data_len -= 30
                pdf.ln(70)
            pdf.output("/home/pi/Desktop/26_final/waste_water_report.pdf")
...
@app.route('/pdf_save')
def pdf_save():
    return send_file("/home/pi/Desktop/26_final/waste_water_report.pdf",mimetype = "application/pdf", as_attachment = True,cache_timeout = -1)
class CustomPDF(FPDF):
 
    def header(self):
        self.image('/home/pi/Desktop/26_final/static/img/water-2.png', 14, 16, 20)
        self.image('/home/pi/Desktop/26_final/static/img/text-1.png', 39, 32, 70)
        self.image('/home/pi/Desktop/26_final/static/img/text-2.png', 154, 32, 30) 
    def footer(self,):
        self.set_y(-50)
...
def send_email():
    try:
        ...
        msg.body = body
        with app.open_resource("/home/pi/Desktop/26_final/waste_water_report.pdf") as fp:
            msg.attach("/home/pi/Desktop/26_final/waste_water_report.pdf", "application/pdf", fp.read())
        mail.send(msg)
...
if __name__ == '__main__':
    ...
    scheduler.start()
    app.run(host='192.168.43.117',port='4444')
